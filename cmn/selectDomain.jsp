...
String checkgroupid  	= Str.CheckNullString((String)ses.getAttribute("login.name")).trim();
String userID     		= Str.CheckNullString((String)ses.getAttribute("login.user")).trim();
String level   			= Str.CheckNullString((String)ses.getAttribute("login.level")).trim();
...
try{
	...
	idList = rootList.getDomain(stmt);	// 데이타 조회
	count = idList.size();
...
<table width="200" border="0" cellpadding="0" cellspacing="0" bgcolor="eaeaea">
  <tr>
    <td height="30" colspan="2" style="padding-left:10;padding-top:5 " background="<%=StaticString.ContextRoot%>/imgs/layer_titlebg_img.gif"> <strong style="font-family:굴림체;font-size:10pt; text-decoration:none; color:RGB(255,255,255);">도메인 선택</strong></td>
    ...
    <td colspan="2" align="center" bgcolor="#FFFFFF" valign="top">
        ...
        <table width="160" border="0" cellspacing="0" cellpadding="0"  class="list_table">
            <tr align="center" height="22" >
		    	<td width="160" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">도 메 인</td>
            </tr>
        </table>
        </div>
...
<%
    ...
    for ( int idx = 0; idx < count ; idx++ ) {
        ...
        if(chk == 0){
%>
            <tr height="22" bgcolor="F3F9F5" align="center" onClick="goAddDomain(this,'<%=commonCodeDTO.getCodeId()%>','<%=formType%>');">
...
%>
            <tr height="22" align="center" onClick="goAddDomain(this,'<%=commonCodeDTO.getCodeId()%>','<%=formType%>');">
              <td width="160" class="table_column"><%=commonCodeDTO.getCodeId()%>&nbsp;</td>
            </tr>
<% 
            chk = 0;
