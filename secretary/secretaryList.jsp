...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(){
		    var parm 	= "";
		    var url 	= 'secretaryInsert.jsp';		    

		    getPage(url,parm);			
		}
		function showEdit(p_E164,p_number){
		    var parm 	= '&e164='+p_E164+'&userparam='+p_number;
		    var url 	= 'secretaryEdit.jsp';
		    if(p_number == ""){
				alert("등록된 인터컴 번호가 없습니다!");
				return;
			}
		...
		function goNewSave(){
	        ...
	        if(value1==""){
				alert("인터컴 수신번호가 입력되지 않았습니다.");
				return;
	        }
	        
			if(value1 != ""){
				if(isNaN(value1)){
					alert("인터컴 수신번호가 숫자가 아닙니다.");
					return;
				}
			}
	        ...
	        f.hiNumber.value = value1;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/secretary/secretaryInsertPro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function goAddPro2(){        
	        ...
	        f.e164.value = srtE164;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/secretary/secretaryEditPro.jsp";
	        f.method="post";
	        f.submit();
	    }
			  ...
			  if(_p=document.getElementById("h"+_i+"_0"))
			  {
		  		...
		  		for(var z=0; z<datas.length; z++){
	 				if(datas[z].params[0]==_e164) {
	 					...
	 					document.getElementById("h"+_i+"_3").innerHTML = document.frm.hiNumber.value ;
	 				}
	 			}
			  }
			  _i++;
		...
		function goDeletePro(){
            var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/secretary/secretaryDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
			  ...
			  if(_p=document.getElementById("h"+_i+"_0"))
			  {
		  		...
		  		for(var z=0; z<datas.length; z++){
	 				if(datas[z].params[0]==_e164) {
	 					...
	 					document.getElementById("h"+_i+"_3").innerHTML = "";
	 				}
	 			}
			  }
			  _i++;
		...
		function func_setAction(num, type) {
			...
			if(type==1){
				goDelete();
				//window.open("secretaryDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert();
				//window.open("secretaryInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
        ...
        <td valign="bottom">
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="150" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">인터컴 수신번호</td>
			...
			if(1==1){		
				...
				if(stmt!=null){
					CommonDAO		commonDao		= new CommonDAO();
									...
									commonSql += "\n,      (select userparam from table_featureservice where e164 = e164.e164 and serviceno = 5121) as userparam  " ;
									commonSql += "\n,      ( select count(*) from table_featureservice where e164 = e164.e164 and serviceno = 5121) as usechk  " ;
					...
					String 			strColNames[]	= {"e164", "extensionnumber","userparam","usechk","totalcnt"};
...
%>	
			  <tr id=g<%=idx%> height="22" bgcolor="E7F0EC" align="center" onmouseover='this.style.backgroundColor="E7F0EC"' onmouseout='this.style.backgroundColor="E7F0EC"' >
                ...
                <%if(flagAlarm>0){%>
	            	<td width="100" id='h<%=idx%>_2' class="table_column"><FONT color="blue">사용중</FONT>&nbsp;</td>
	            <%}else{%>
					<td width="100" id='h<%=idx%>_2' class="table_column">사용안함&nbsp;</td>
	            <%}%>
	            	<td width="150" id='h<%=idx%>_3' class="table_column"><%=values[idx][1]%>&nbsp;</td>
                <td width="80" id='h<%=idx%>_4' class="table_column"><input type="button" name="btnAction" style="height: 18px" value="수정" onclick="showEdit('<%=values[idx][0]%>', '<%=values[idx][1]%>')"></td>
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="secretaryList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="secretaryList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"secretaryList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="secretaryList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="secretaryList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
