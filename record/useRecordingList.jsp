...
<%
	...
	if(nModeDebug!=1){
		try{
			if(nAllowUser==1){		
				...
				if(stmt!=null){
					...
					iList = new ArrayList<FeatureServiceDTO>();
					String sql = "\n select a.e164 as e164 "; 
			        //sql += "\n 	, b.endpointid as endpointid, b.zonecode as zonecode  ";
			        sql += "\n 	, b.serviceno as serviceno ";
			        sql += "\n 	, a.extensionnumber as extensionnumber ";
			        sql += "\n from table_e164 a "; 
			        //sql += "\n left outer join table_sipendpoint b on b.endpointid like a.e164||'@%' and a.groupid=b.checkgroupid ";
			        sql += "\n left outer join table_featureservice b on b.e164=a.e164 and serviceno = 5511 ";
			        
			        if(authGroupid!=null) sql += "\n WHERE a.groupid = '"+authGroupid+"' ";
			        sql += "\n order by a.e164 ";  
			        ...
			        FeatureServiceDTO sipendpointDTO; 
			        ResultSet rs = null;
			        try{
			                rs = stmt.executeQuery(sql);
			                System.out.println("useRecording : "+sql);
			                while (rs.next()) {
			                	sipendpointDTO = new FeatureServiceDTO();
			                	sipendpointDTO.setE164(Str.CheckNullString(rs.getString("e164")));
			                	//sipendpointDTO.setZoneCode(Str.CheckNullString(rs.getString("zonecode")));
			                	sipendpointDTO.setServiceNo(Str.CheckNullInt(rs.getString("serviceno")));
			                	sipendpointDTO.setUserParam(Str.CheckNullString(rs.getString("extensionnumber")));//extensionnumber        	
			                	iList.add(sipendpointDTO);
			                }
			        }catch(Exception ex){
			        }finally{
						if(rs!=null) rs.close();			        	
					...
					try{
			        	if(nModePaging==1){
			        		sql = "select count(*) ";
			        		sql += "\n from table_e164 a left outer join table_featureservice b on b.e164=a.e164 and serviceno = 5511 ";
					        if(authGroupid!=null) sql += "\n WHERE a.groupid = '"+authGroupid+"' ";
					        rs = stmt.executeQuery(sql);
		...
		iList = new ArrayList<FeatureServiceDTO>() ;
		FeatureServiceDTO tmpdto = null;
		for(int z=0; z<nMaxitemcnt; z++){
			tmpdto = new FeatureServiceDTO();
			tmpdto.setE164("0101111222"+z) ;
			//tmpdto.setEndpointID("0101111222"+z+"@~") ;
			tmpdto.setServiceNo(z%2==0? 5511:0) ;// serviceno ['':사용안함, '5511':사용중], zonecode['':사용안함, 'Z0001':사용중]
...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(p_E164, p_Param){
		    ...
		    var url 	= 'useRecordingInsert.jsp';//'useRecordingInsert.jsp';
	    ...
	    function editPro(e164s){
	    	...
	    	var url 	= 'useRecordingInsertPro.jsp';
	    	f2.e164.value = e164s;
	    	
	    	// 신규 저장
	        f2.target="procframe";
	        f2.action="<%=StaticString.ContextRoot+pageDir%>/record/useRecordingInsertPro.jsp";
	        f2.method="post";
	        f2.submit();
		}
		/**
        ...
        function goDelete(){
            var parm 	= '';
            var url 	= 'useRecordingDelete.jsp';		    
            getPage(url,parm);			
        }
		...
		function goDeletePro(){
            var f   = document.frm;
		   	...
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/record/useRecordingDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
            ...
            if(f.gubun[0].checked){     // 지정 시간 통보 사용하지 않음 
                if(f.chkOpt == undefined){
                    ...
                    if(confirm("모든 설정이 삭제됩니다. 저장하시겠습니까?")){
                        f.target = "procframe";
                        f.action = "<%=StaticString.ContextRoot+pageDir%>/record/useRecordingSavePro.jsp";
                        f.method = "post";
                        f.submit();	
                    }                
                }
		...
		function setE164BySelected(){
            ...
            if(f != undefined && f.chkOpt != undefined){
                if(f.chkOpt.length == undefined){
                    if(f.chkOpt.checked){
                    	f.chkOpt.value = f.chkOpt.parentNode.parentNode.cells[1].innerHTML;
                        str = f.chkOpt.value;
                    }
                }else{
                    for(var i=0; i<f.chkOpt.length; i++){
                        if(f.chkOpt[i].checked){
                            if(str == ""){
                            	f.chkOpt[i].value = f.chkOpt[i].parentNode.parentNode.cells[1].innerHTML;
                                str = f.chkOpt[i].value;
                            }else{
                            	f.chkOpt[i].value = f.chkOpt[i].parentNode.parentNode.cells[1].innerHTML;
                                str = str + "" + f.chkOpt[i].value;
		...
		function func_setAction(num, virtual, action) {
			...
			if(action==1){
				goDelete();
				//window.open("useRecordingDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert(num, virtual);
				//window.open("useRecordingInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
...
// 		 	document.cookie = "id_cookie_callblock" + "=";
		...
		function realtimeClock() {
			  document.location.href = 'useRecordingList.jsp';
          ...
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  %>
                	 	...
                	 	<input type="button" style="height: 18px" value="갱신" onclick="document.location.href = 'useRecordingList.jsp'">
	           	  <% }
                  	else{ 
	           			//out.println("<input type=\"button\" name=\"btnLogin\" style=\"height: 18px\" value=\"로그인\" onclick=\"document.location.href = '.'\">") ;
	           	  %>
                  		<input type="button" name="btnLogin" style="height: 18px" value="로그인" onclick="document.location.href = 'useRecordingList.jsp'"> 
	           	  <% } %> --%>
                  </td>
                  <td colspan="3"></td>
        ...
        <td valign="bottom">
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="197" onclick="sortNow(1);changeTitle(1);" id="extnum" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">내선번호<font size='1px'>▽</font></td>
...
<%					
	...
	if(/* envList!=null && */ count!=0){
		...
		FeatureServiceDTO dto= null;
		int usedFlag = 0;
		//String[] strVirtuals;
		if(iList!=null)
			for(idx=0;idx<endidx;idx++){
				dto	= (FeatureServiceDTO)iList.get(idx);
				if(dto!=null){
					usedFlag = dto.getServiceNo()==5511 ? 1:0 ;
					...
					%>	
					  <tr id=g<%=idx%> height="22" align="center" bgcolor="<%=idx%2==0?"#F3F9F5":"#fcfcfc"%>" onmouseover='this.style.backgroundColor="#E7F0EC"' onmouseout='<%=idx%2==0?"this.style.backgroundColor=\"#F3F9F5\"":"this.style.backgroundColor=\"#fcfcfc\"" %>' >
		                ...
		                <td width="300" id='h<%=idx%>_0' class="table_column"><%=dto.getE164()%></td>
		                <td width="197" id='h<%=idx%>_1' class="table_column"><%=dto.getUserParam()/*extensionnumber*/%></td>
		                <td width="80" id='h<%=idx%>_2' class="table_column">
		                <%
			                if( usedFlag==0 ) out.print("사용안함&nbsp;");
			                else out.print("<FONT color=\"blue\">사용중</FONT>&nbsp;");
		                %>
		                </td>
		                <td width="80" id='h<%=idx%>_3' class="table_column">
		                	<input type="button" name="btnAction" style="height: 18px" value="<%=usedFlag==0?"등록":"삭제"%>" onclick="func_setAction('<%=dto.getE164()%>', '', <%=usedFlag==0?0:1%>)">
		                </td>
		                <td class="table_column" width="10">&nbsp;</td>
		              </tr>
					<% 
				}
			}//for
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="useRecordingjsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="useRecordingList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"useRecordingList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="useRecordingList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="useRecordingList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
