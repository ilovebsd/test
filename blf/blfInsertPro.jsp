    ...
    for(String strE164 : e164s)
    	if( (strE164 = strE164.trim()).length()>0 ){
    		if(jsArray.length()==0)	jsArray.append("[{params:");
	            	...
	            	sql  = " SELECT count(*) FROM table_featureservice WHERE e164 = '" + strE164 +"' and serviceno = 5421 ";
                    ...
                    if (cnt == 0){
		    			...
		    			sql = sql + "  values('" + strE164 + "', 5421, 5421, '', 2, 1, 0, 2)";
	        			System.out.println(sql);
                        nResult = statement.executeUpdate(sql);
                        if (nResult < 1){	throw new Exception(l.x("[AlertInfo 등록 오류] '","[AlertInfo] '")+strE164+l.x("'는 BLF등록이 실패하였습니다.","' Insert Fail"));	}            
                    }
                    count++ ;
