...
<script language="JavaScript" type="text/JavaScript">
		...
		function goNewSave(){
	        var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/blf/blfInsertPro.jsp";
            f.method = "post";
            f.submit();	
		}		
		/**
		...
		function goDeletePro(){
            var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/blf/blfDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function func_setAlarm(num, type) {
			...
			if(type==1){
				goDelete();
				//window.open("blfDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert();
				//window.open("blfInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
		...
		// 		String 			strColNames[]	= {"sysgroupid","sysgroupname"};
				...
				if(stmt!=null){
					CommonDAO		commonDao		= new CommonDAO();
									...
									sql += "\n,      ( select count(*) from table_featureservice where e164 = e164.e164 and serviceno = 5421) as usechk  " ;
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="blfList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="blfList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"blfList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="blfList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="blfList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
