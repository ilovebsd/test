...
<script language="JavaScript" type="text/JavaScript">
		...
		function goNewSave(){
	        var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/vms/vmsInsertPro.jsp";
            f.method = "post";
            f.submit();	
		}		
		/**
		...
		function goDeletePro(){
            var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/vms/vmsDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function func_setAlarm(num, type) {
			...
			if(type==1){
				goDelete();
				//window.open("vmsDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert();
				//window.open("vmsInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
...
<%					
	...
	if(1==1){
		...
		// 		String 			sql		= "SELECT sysgroupid, sysgroupname FROM table_systemgroup";
		// 		String 			strColNames[]	= {"sysgroupid","sysgroupname"};
				String sesSysGroupID = StringUtil.null2Str(request.getSession(false).getAttribute("login.sysgroupid"), "");
				stmt 			= ConnectionManager.allocStatement("SSW", sesSysGroupID);
				if(stmt!=null){
					CommonDAO		commonDao		= new CommonDAO();
						 			sql		= "select e164, extensionnumber, (select count(*) from nasa_vms_user where vms_id = e164) as usechk " ; 
									sql		+= ", ( SELECT count(*) FROM table_e164 WHERE groupid = '"+authGroupid+"' ) as totalcnt ";
									sql		+= "From Table_e164  WHERE groupid = '"+authGroupid+"'";
					
					sql		+= " ORDER BY extensionnumber is null ASC, extensionnumber='' ASC, cast('0'||extensionnumber as int) ASC " ;
					...
					envList2		= commonDao.select(stmt,sql,strColNames);
					if(envList2.size()==0){
			...
			out.println("<script type=\"text/JavaScript\"> alert('"+sql+"');</script>") ;
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="vmsList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="vmsList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"vmsList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="vmsList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="vmsList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
