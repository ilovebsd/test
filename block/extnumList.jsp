			...
			try{
				if(nAllowUser==1){		
					...
					if(stmt!=null){
						...
						commonSql		+= " ORDER BY cast(extensionnumber as int) ASC " ;
				        ...
				        try {
				                ...
				                System.out.println("SQL : ExtNum "+sql);
				                while (rs.next()) {
				                	mrbtDTO = new MrbtDTO();
  ...
  if (document.loginForm.id.value == "") {
     ...
     document.loginForm.pwd.focus();
      ...
      if(document.loginForm.id_save.checked){
        SetCookie("id_cookie_extnum",document.loginForm.id.value, 90); //쿠키값 하루 설정
      }else{
        delCookie("id_cookie_extnum",document.loginForm.id.value);
      }
...
function func_init_callblock() {
	...
	//if(groupid) SetCookie("id_cookie_extnum", groupid, 90); //쿠키값 하루 설정
	//delCookie("id_cookie_extnum", '');
}
...
<script language="JavaScript" type="text/JavaScript">
		...
		function goEdit(){
			...
			var tag;                 
            var i=0;
            while( tag = document.getElementById("h"+i+"_0") ){
            	num = tag.innerHTML;
            	tag = document.getElementById("h"+i+"_new") ;
            	value = tag.value; 
                if( !value || value.trim().length==0){
                	alert("입력되니 않은 내선번호 값이 존재합니다!");
                    return;
                }else{
    		...
    		Ext.Ajax.request({
    			...
    			success: function ( result, request ) {
    		    	...
    		    	if(value=='OK'){ 
    			    	...
    			    	alert(value.substr(3)+"의 내선번호가 이미 존재하는 값입니다!");
    			    	//document.location.href = 'extnumList.jsp';
    			    	return;		
    			    }else{
    			    	alert("실패!");
    			    	return;
		...
		function goEditPro(parms){
			
			//var parm 	= '&insertStr='+str;		//get형식으로 변수 전달.
    	    var url 	= 'extnumEditPro.jsp';//'extnumChk.jsp';
    	    //alert(parms);
    		Ext.Ajax.request({
    			url : url , 
    			params : parms,
...
// 		 	document.cookie = "id_cookie_extnum" + "=";
...
<FORM name="frm" method="post">
	...
	<tr>
        <td valign="bottom">
          <table width="350" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  %>
                	 	...
                	 	<input type="button" style="height: 18px" value="갱신" onclick="document.location.href = 'extnumList.jsp'">
	           	  <% }
                  	else{ 
	           			//out.println("<input type=\"button\" name=\"btnLogin\" style=\"height: 18px\" value=\"로그인\" onclick=\"document.location.href = '.'\">") ;
	           	  %>
                  		<input type="button" name="btnLogin" style="height: 18px" value="로그인" onclick="document.location.href = 'extnumList.jsp'"> 
	           	  <% } %> --%>
                  </td>
                  <td colspan="3"></td>
        ...
        <td valign="bottom">
          <table width="350" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="230" id="memname" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">내선번호</td>
					...
					%>	
					  <tr id=g<%=idx%> height="22" align="center" bgcolor="<%=idx%2==0?"#F3F9F5":"#fcfcfc"%>" onmouseover='this.style.backgroundColor="#E7F0EC"' onmouseout='<%=idx%2==0?"this.style.backgroundColor=\"#F3F9F5\"":"this.style.backgroundColor=\"#fcfcfc\"" %>' >
		                ...
		                <td width="230" id='h<%=idx%>_1' class="table_column">
		                	<input type="text" id='h<%=idx%>_new' value='<%=dto.getName()%>' />
						</td>
		                ...
		                <input type='hidden' id='h<%=idx%>_old' value='<%=dto.getName()%>' />
		              </tr>
					<% 
				}
			}//for
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<%-- <td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="extnumList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="extnumList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td> --%>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"extnumList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 		       					
		       					</td>
		       					<td>
		       			<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="extnumList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="extnumList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		       					</td>
