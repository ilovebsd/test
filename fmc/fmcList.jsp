...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(){
		    var parm 	= "";
		    var url 	= 'fmcInsert.jsp';		    

		    getPage(url,parm);			
		}
		function showEdit(p_E164,p_number){
		    var parm 	= '&e164='+p_E164+'&userparam='+p_number;
		    var url 	= 'fmcEdit.jsp';
		    if(p_number == ""){
				alert("등록된 입력값이 없습니다!");
				return;
		...
		function goNewSave(){
			...
			var srt1 = document.Savelayer.txtNumber.value;
	        var value1 	= srt1.replace(/\s/g, "");	// 공백제거
	        if(value1==""){
	        ...
	        f.hiNumber.value = value1;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/fmc/fmcInsertPro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function goAddPro2(){        
	        ...
	        var srtE164	= document.editForm.e164.value;
			var srt1 = document.editForm.txtNumber.value;
			var value1 	= srt1.replace(/\s/g, "");	// 공백제거
	        ...
	        var f   		= document.frm;
	        f.hiNumber.value = value1;
	        f.e164.value = srtE164;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/fmc/fmcEditPro.jsp";
	        f.method="post";
	        f.submit();
	    }
			  ...
			  if(_p=document.getElementById("h"+_i+"_0"))
			  {
		  		...
		  		for(var z=0; z<datas.length; z++){
	 				if(datas[z].params[0]==_e164) {
	 					document.getElementById("h"+_i+"_1").innerHTML = "<FONT color=\"blue\">사용중</FONT>&nbsp;" ;
	 					document.getElementById("h"+_i+"_2").innerHTML = document.frm.hiNumber.value ;
	 					document.getElementById("h"+_i+"_3").innerHTML = "<input type='button' name='btnAction' style='height: 18px' value='수정' onclick=\"showEdit('"+_e164+"', '"+document.frm.hiNumber.value+"')\">" ;
	 				}
	 			}
			  }
			  _i++;
		...
		function goDeletePro(){
            var f   = document.frm;
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/fmc/fmcDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function goDeleteDone(datas){
			...
			while(_o=document.getElementById("g"+_i))
			{
			  ...
			  if(_p=document.getElementById("h"+_i+"_0"))
			  {
		  		...
		  		for(var z=0; z<datas.length; z++){
	 				if(datas[z].params[0]==_e164) {
	 					document.getElementById("h"+_i+"_1").innerHTML = "사용안함&nbsp;" ;
	 					document.getElementById("h"+_i+"_2").innerHTML = "";
	 					document.getElementById("h"+_i+"_3").innerHTML = "<input type='button' name='btnAction' style='height: 18px' value='수정' onclick=\"showEdit('"+_e164+"', '')\">" ;
	 				}
	 			}
			  }
			  _i++;
         ...
         * 추가/해제 클릭
         */
		function func_setAction(num, type) {
			document.frm.e164.value = num;
			document.frm.grpid.value = '<%=authGroupid%>';
			if(type==1){
				goDelete();
				//window.open("fmcDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert();
				//window.open("fmcInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
        ...
        <td valign="bottom">
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="150" onclick="sortNow(0,true);changeTitle(0);" id="telnum" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">전화번호<font size='1px'>▽</font></td>
                  ...
                  <td width="230" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">입력값</td>
...
<%					
	...
	try{
		...
		if(stmt!=null){
			...
			commonSql = "\n SELECT e.e164 AS e164"; 
	        commonSql += "\n ,(Select count(*) From table_featureservice Where e164 = e.e164 And serviceno = 5501 And checkgroupid = '"+authGroupid+"') AS usechk  ";
	        commonSql += "\n ,(Select userparam From table_featureservice Where e164 = e.e164 And serviceno = 5501 And checkgroupid = '"+authGroupid+"') AS userparam  " ;
	      	//commonSql += "\n , f.userparam as userparam  ";
	        //commonSql += "\n FROM table_e164 e LEFT OUTER JOIN table_featureservice f ON e.e164=f.e164 AND f.serviceno = 5501 AND e.checkgroupid=f.checkgroupid ";
	        commonSql += "\n FROM table_e164 e " ;
	        commonSql += "\n WHERE e.checkgroupid = '"+authGroupid+"' " ;
	        commonSql += "\n ORDER BY e.e164 "; 
			...
			String 			strColNames[]	= {"e164", "userparam","usechk"};
			
			System.out.println("FMV : "+commonSql);
			...
			try{
	        	if(nModePaging==1){
	        		commonSql = "\n select count(*) ";
	        		commonSql += "\n From table_e164 WHERE checkgroupid = '"+authGroupid+"' " ;
...
%>	
			  ...
			  <tr id=g<%=idx%> height="22" align="center" bgcolor="<%=idx%2==0?"#F3F9F5":"#fcfcfc"%>" onmouseover='this.style.backgroundColor="#E7F0EC"' onmouseout='<%=idx%2==0?"this.style.backgroundColor=\"#F3F9F5\"":"this.style.backgroundColor=\"#fcfcfc\"" %>' >
                ...
                <%if(flagAlarm>0){%>
	            	<td width="100" id='h<%=idx%>_1' class="table_column"><FONT color="blue">사용중</FONT>&nbsp;</td>
	            <%}else{%>
					<td width="100" id='h<%=idx%>_1' class="table_column">사용안함&nbsp;</td>
	            <%}%>
	            <td width="230" id='h<%=idx%>_2' class="table_column"><%=values[idx][1]%>&nbsp;</td>
                <td width="80" id='h<%=idx%>_3' class="table_column"><input type="button" name="btnAction" style="height: 18px" value="수정" onclick="showEdit('<%=values[idx][0]%>', '<%=values[idx][1]%>')"></td>
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="fmcList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="fmcList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"fmcList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="fmcList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="fmcList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
