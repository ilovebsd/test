		...
		try{
			if(nAllowUser==1){		
				...
				if(stmt!=null){
					...
					//iList	= (ArrayList)addServiceList.getDeptMRBTList(stmt, authGroupid);		// 데이타 조회
					MrbtDTO mrbtDTO;
			        ...
			        sql = sql +  "\n 	(select extensionnumber from TABLE_e164 where e164 = b.keynumberid) as extnum, ";
...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(p_E164, p_file){
		    ...
		    var url 	= 'deptMrbtEdit.jsp';//'deptMrbtInsert.jsp';
		    if(p_file == ""){
				alert("등록된 연결음이 없습니다!");
				return;
			}
...
//			f2.insertStr_02.value 		= str;
	    	...
	    	var url 	= 'deptMrbtEditPro.jsp';
	    	
	    	// 신규 저장
	        f2.target="procframe";
	        f2.action="<%=StaticString.ContextRoot+pageDir%>/deptmrbt/deptMrbtEditPro.jsp";
	        f2.method="post";
	        f2.submit();
		...
		function goDeletePro(){
            ...
            //alert('e164s='+str);
		   	...
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/deptmrbt/deptMrbtDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
			  ...
			  if(_p=document.getElementById("h"+_i+"_0"))
			  {
		  		...
		  		for(var z=0; z<datas.length; z++){
	 				if(datas[z].params[0]==_e164) {
	 					...
	 					_td = document.getElementById("h"+_i+"_4") ;
	 					if(_td){
	 						//_td.innerHTML = "<input type=\"button\" name=\"btnAction\" style=\"height: 18px\" value=\"수정\" onclick=\"func_setAction('"+_e164+"', 0, 0)\" >";
...
//			f2.insertStr_02.value 		= str;
	    	...
	    	// 신규 저장
	        f2.target="procframe";
	        f2.action="<%=StaticString.ContextRoot+pageDir%>/deptmrbt/deptMrbtEditAddPro.jsp";
	        f2.method="post";
	        f2.submit();
		}
		/**
		...
		function showEditDelete(p_E164, p_day, p_start, p_end, p_file){
			...
			f.target='procframe';
	        //f.action="<%=StaticString.ContextRoot%>/mrbtDelete4.do";
	        f.action="<%=StaticString.ContextRoot+pageDir%>/deptmrbt/deptMrbtEditDeletePro.jsp";
	        f.method="post";
	        f.submit();		
		}
            ...
            if(f.gubun[0].checked){     // 지정 시간 통보 사용하지 않음 
                if(f.chkOpt == undefined){
                    ...
                    if(confirm("모든 설정이 삭제됩니다. 저장하시겠습니까?")){
                        f.target = "procframe";
                        f.action = "<%=StaticString.ContextRoot+pageDir%>/deptmrbt/deptMrbtSavePro.jsp";
                        f.method = "post";
                        f.submit();	
                    }                
                }
		...
		function isChecked(processname){
            ...
            var cnt = 0;
            if(f.chkOpt == undefined){
                var parm = '&titlemsg='+'대표통화 연글음 설정'+'&msg='+'검색 목록이 없습니다.';
                var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                getPage(url,parm);
                return 0;
            }else{
                ...
                if(cnt == 0){
                    var parm = '&titlemsg='+'대표통화 연글음 설정'+'&msg='+processname+'할 항목을 선택하여 주십시오.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return 0;
                }
		...
		function func_setAction(num, virtual, action) {
			...
			if(action==1){
				goDelete();
				//window.open("deptMrbtDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert(num, virtual);
				//window.open("deptMrbtInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
...
// 		 	document.cookie = "id_cookie_callblock" + "=";
		...
		function realtimeClock() {
			  document.location.href = 'deptMrbtList.jsp';
		...
		function changeTitle(nField){
			if(nField==0){
				if( lastSort==nField ){
					document.getElementById('telnum').innerHTML = "<b>대표번호▲</b>";
				}else{
					document.getElementById('telnum').innerHTML = "<b>대표번호▼</b>";							
				}
				document.getElementById('dept').innerHTML = "내선번호<font size='1px'>▽</font>";
				document.getElementById('filename').innerHTML = "연결음 파일명<font size='1px'>▽</font>";
			}
			else if(nField==1){
				if( lastSort==nField ){
					document.getElementById('dept').innerHTML = "<b>내선번호▲</b>";
				}else{
					document.getElementById('dept').innerHTML = "<b>내선번호▼</b>";							
				}
				document.getElementById('telnum').innerHTML = "대표번호<font size='1px'>▽</font>";
				document.getElementById('filename').innerHTML = "연결음 파일명<font size='1px'>▽</font>";
          ...
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  %>
                	 	...
                	 	<input type="button" style="height: 18px" value="갱신" onclick="document.location.href = 'deptMrbtList.jsp'">
	           	  <% }
                  	else{ 
	           			//out.println("<input type=\"button\" name=\"btnLogin\" style=\"height: 18px\" value=\"로그인\" onclick=\"document.location.href = '.'\">") ;
	           	  %>
                  		<input type="button" name="btnLogin" style="height: 18px" value="로그인" onclick="document.location.href = 'deptMrbtList.jsp'"> 
	           	  <% } %> --%>
                  </td>
                  <td colspan="3"></td>
        ...
        <td valign="bottom">
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="150" onclick="sortNow(0,true);changeTitle(0);" id="telnum" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif"><b>대표번호▲</b></td>
                  <td width="150" onclick="sortNow(1,true);changeTitle(1);" id="dept" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">내선번호<font size='1px'>▽</font></td>
                  <td width="180" onclick="sortNow(2);changeTitle(2);" id="filename" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">연결음 파일명<font size='1px'>▽</font></td>
                  <td width="100" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">사용유무</td>
				...
				if(dto!=null){
					...
					%>	
					  <tr id=g<%=idx%> height="22" align="center" bgcolor="<%=idx%2==0?"#F3F9F5":"#fcfcfc"%>" onmouseover='this.style.backgroundColor="#E7F0EC"' onmouseout='<%=idx%2==0?"this.style.backgroundColor=\"#F3F9F5\"":"this.style.backgroundColor=\"#fcfcfc\"" %>' >
		                ...
		                <td width="150" id='h<%=idx%>_0' class="table_column"><%=dto.getE164()%></td>
		                <td width="150" id='h<%=idx%>_1' class="table_column"><%=dto.getName()%></td>
		                <td width="180"	id='h<%=idx%>_2' class='table_column'><%=1>dto.getUsechk()? "&nbsp;":"<a href=\"javascript:goSample('"+dto.getFilename()+"');\">"+dto.getFilename()+"&nbsp;</a>" %></td>
		                ...
		                <td width="35" id='h<%=idx%>_4' class="table_column">
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="deptMrbtList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="deptMrbtList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"deptMrbtList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="deptMrbtList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="deptMrbtList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
