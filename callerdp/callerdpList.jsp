...
// 			        sql = sql +  "\n 	b.position as position,  ";
			        sql = sql +  "\n 	(Select count(*) From Table_FeatureService Where ServiceNo = 5561 And e164 = b.phonenum ) as usechk ";
			        ...
			        try{
			                ...
			                System.out.println("callerdp : "+sql);
			                while (rs.next()) {
			                	mrbtDTO = new MrbtDTO();
...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(p_E164, p_Param){
		    ...
		    var url 	= 'callerdpInsert.jsp';//'callerdpInsert.jsp';
	    ...
	    function editPro(e164s){
	    	...
	    	var url 	= 'callerdpInsertPro.jsp';
	    	f2.e164.value = e164s;
	    	
	    	// 신규 저장
	        f2.target="procframe";
	        f2.action="<%=StaticString.ContextRoot+pageDir%>/callerdp/callerdpInsertPro.jsp";
	        f2.method="post";
	        f2.submit();
		}
		/**
		...
		function goDeletePro(){
            var f   = document.frm;
		   	...
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/callerdp/callerdpDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function goSave(){
            ...
            if(f.gubun[0].checked){     // 지정 시간 통보 사용하지 않음 
                if(f.chkOpt == undefined){
                    var parm = '&titlemsg='+'발신자 번호 표시 설정'+'&msg='+'발신자 번호 표시를 사용하지 않는 상태입니다.';
                    ...
                    if(confirm("모든 설정이 삭제됩니다. 저장하시겠습니까?")){
                        f.target = "procframe";
                        f.action = "<%=StaticString.ContextRoot+pageDir%>/callerdp/callerdpSavePro.jsp";
                        f.method = "post";
                        f.submit();	
                    }                
                }
                ...
                if(f.chkOpt == undefined){
                    var parm = '&titlemsg='+'발신자 번호 표시 설정'+'&msg='+'발신자 번호 표시 설정을 추가하여 주십시오.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return;
                }else{
                    var parm = '&titlemsg='+'발신자 번호 표시 설정'+'&msg='+'발신자 번호 표시를 사용하고 있습니다.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return;
                }
		...
		function isChecked(processname){
            ...
            var cnt = 0;
            if(f.chkOpt == undefined){
                var parm = '&titlemsg='+'발신자 번호 표시 설정'+'&msg='+'검색 목록이 없습니다.';
                var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                getPage(url,parm);
                return 0;
            }else{
                ...
                if(cnt == 0){
                    var parm = '&titlemsg='+'발신자 번호 표시 설정'+'&msg='+processname+'할 항목을 선택하여 주십시오.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return 0;
                }
		...
		function func_setAction(num, virtual, action) {
			...
			if(action==1){
				goDelete();
				//window.open("callerdpDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert(num, virtual);
				//window.open("callerdpInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
...
// 		 	document.cookie = "id_cookie_callblock" + "=";
		...
		function realtimeClock() {
			  document.location.href = 'callerdpList.jsp';
          ...
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  %>
                	 	...
                	 	<input type="button" style="height: 18px" value="갱신" onclick="document.location.href = 'callerdpList.jsp'">
	           	  <% }
                  	else{ 
	           			//out.println("<input type=\"button\" name=\"btnLogin\" style=\"height: 18px\" value=\"로그인\" onclick=\"document.location.href = '.'\">") ;
	           	  %>
                  		<input type="button" name="btnLogin" style="height: 18px" value="로그인" onclick="document.location.href = 'callerdpList.jsp'"> 
	           	  <% } %> --%>
                  </td>
                  <td colspan="3"></td>
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="callerdpList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="callerdpList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"callerdpList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="callerdpList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="callerdpList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
