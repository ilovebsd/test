		...
		try{
			if(nAllowUser==1){		
				...
				if(stmt!=null){
					...
					//iList	= (ArrayList)addServiceList.getNewMOHList(stmt, authGroupid);		// 데이타 조회
...
<script language="JavaScript" type="text/JavaScript">
		...
		function goInsert(p_E164, p_file){
		    ...
		    var url 	= 'mohEdit.jsp';//'mohInsert.jsp';
		    if(p_file == ""){
				alert("등록된 대기음이 없습니다!");
				return;
			}
		...
		function goInsertPro(){
            ...
            f.uploadfilename.value 	= uploadfile;
         	...
         	// 신규 부서대표번호 저장
    	    var parm 	= '&hie164='+str+'&uploadfilename='+uploadfile;		//get형식으로 변수 전달.
    	    var url 	= 'mohChk.jsp';
	    ...
	    function editPro(e164s){
	    	...
	    	var url 	= 'mohEditPro.jsp';
	    	
	    	// 신규 저장
	        f2.target="procframe";
	        f2.action="<%=StaticString.ContextRoot+pageDir%>/moh/mohEditPro.jsp";
	        f2.method="post";
	        f2.submit();
		}
		/**
		...
		function goDeletePro(){
            ...
            //alert('e164s='+str);
		   	...
		   	f.target = "procframe";
            f.action = "<%=StaticString.ContextRoot+pageDir%>/moh/mohDeletePro.jsp";
            f.method = "post";
            f.submit();	
		}
		...
		function goSave(){
            ...
            if(f.gubun[0].checked){     // 지정 시간 통보 사용하지 않음 
                if(f.chkOpt == undefined){
                    var parm = '&titlemsg='+'통화대기음 설정'+'&msg='+'통화대기음를 사용하지 않는 상태입니다.';
                    ...
                    if(confirm("모든 설정이 삭제됩니다. 저장하시겠습니까?")){
                        f.target = "procframe";
                        f.action = "<%=StaticString.ContextRoot+pageDir%>/moh/mohSavePro.jsp";
                        f.method = "post";
                        f.submit();	
                    }                
                }
                ...
                if(f.chkOpt == undefined){
                    var parm = '&titlemsg='+'통화대기음 설정'+'&msg='+'통화대기음 설정을 추가하여 주십시오.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return;
                }else{
                    var parm = '&titlemsg='+'통화대기음 설정'+'&msg='+'통화대기음를 사용하고 있습니다.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return;
                }
		...
		function isChecked(processname){
            ...
            var cnt = 0;
            if(f.chkOpt == undefined){
                var parm = '&titlemsg='+'통화대기음 설정'+'&msg='+'검색 목록이 없습니다.';
                var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                getPage(url,parm);
                return 0;
            }else{
                ...
                if(cnt == 0){
                    var parm = '&titlemsg='+'통화대기음 설정'+'&msg='+processname+'할 항목을 선택하여 주십시오.';
                    var url  = "<%=StaticString.ContextRoot+pageDir%>/msgPopup.jsp";
                    getPage(url,parm);
                    return 0;
                }
		...
		function func_setAction(num, virtual, action) {
			...
			if(action==1){
				goDelete();
				//window.open("mohDelete.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
			}
			else{
				goInsert(num, virtual);
				//window.open("mohInsert.jsp?num="+num, "_blank", "width=430,height=260,resizable=1,scrollbars=1,menubar=0,location=0,toolbar=0,status=0,directories=0") ;
        	}
		}
...
// 		 	document.cookie = "id_cookie_callblock" + "=";
		...
		function realtimeClock() {
			  document.location.href = 'mohList.jsp';
		...
		function changeTitle(nField){
			if(nField==0){
				...
				document.getElementById('filename').innerHTML = "대기음 파일명<font size='1px'>▽</font>";
			}
			else if(nField==1){
				if( lastSort==nField ){
				...
				if( lastSort==nField ){
					document.getElementById('filename').innerHTML = "<b>대기음 파일명▲</b>";
				}else{
					document.getElementById('filename').innerHTML = "<b>대기음 파일명▼</b>";							
				}
				document.getElementById('memname').innerHTML = "내선번호<font size='1px'>▽</font>";							
          ...
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  %>
                	 	...
                	 	<input type="button" style="height: 18px" value="갱신" onclick="document.location.href = 'mohList.jsp'">
	           	  <% }
                  	else{ 
	           			//out.println("<input type=\"button\" name=\"btnLogin\" style=\"height: 18px\" value=\"로그인\" onclick=\"document.location.href = '.'\">") ;
	           	  %>
                  		<input type="button" name="btnLogin" style="height: 18px" value="로그인" onclick="document.location.href = 'mohList.jsp'"> 
	           	  <% } %> --%>
                  </td>
                  <td colspan="3"></td>
        ...
        <td valign="bottom">
          <table width="700" border="0" cellspacing="0" cellpadding="0" align="left" bgcolor="ffffff" style="border:1 solid rgb(160,160,160) ">
              <tr align="center" height="22" >
                  ...
                  <td width="150" onclick="sortNow(0,true);changeTitle(0);" id="memname" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif"><b>내선번호▲</b></td>
                  <td width="150" onclick="sortNow(1,true);changeTitle(1);" id="telnum" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">전화번호<font size='1px'>▽</font></td>
                  <td width="180" onclick="sortNow(2);changeTitle(2);" id="filename" class="table_header01" background="<%=StaticString.ContextRoot%>/imgs/table_header_img.gif">대기음 파일명<font size='1px'>▽</font></td>
				...
				if(dto!=null){
					...
					%>	
					  <tr id=g<%=idx%> height="22" align="center" bgcolor="<%=idx%2==0?"#F3F9F5":"#fcfcfc"%>" onmouseover='this.style.backgroundColor="#E7F0EC"' onmouseout='<%=idx%2==0?"this.style.backgroundColor=\"#F3F9F5\"":"this.style.backgroundColor=\"#fcfcfc\"" %>' >
		                <td width="43" class="table_column"> <input type="checkbox" name="chkOpt" onclick="this.value=this.parentNode.parentNode.cells[3].innerHTML;" value="<%=StringUtil.nvl(dto.getE164())%>" > </td> <%-- rgb(243,247,245)==3F7F5 or A8D3AA --%>
		                <td width="43" class="table_column"><%=nModePaging==1? nNowpage*nMaxitemcnt+idx+1 : idx+1 %></td>
		                <td width="150" id='h<%=idx%>_0' class="table_column"><%=dto.getName()%></td>
		                <td width="150" id='h<%=idx%>_1' class="table_column"><%=dto.getE164()%></td>
		       ...
		       <tr height="22" bgcolor="E7F0EC" align="center" >
		       		<td colspan = 2 align="right" > 
		       			<% if(nBlockidx > 0){ %>
		       				<table width="50">
		       					<tr>
		       						<td align="left"> <a href="mohList.jsp?page=0"> &#60;&#60; </a> </td>
		       						<td align="right"> <a href="mohList.jsp?page=<%=(nBlockidx-1)*nBlockcnt+nBlockcnt-1%>"> &#60; </a> </td>
		       					</tr>
		       				</table> 
		       			<% } %>
		       		</td>
...
<%					
		for(int i=(nBlockidx*nBlockcnt); i<(nBlockidx+1)*nBlockcnt && i<=nTotalpage; i++){
      			if(nNowpage==i)
      				...
      				out.print(" <a href=\"mohList.jsp?page="+(i)+"\"> "+(i+1)+" </a> ") ;
		}
%> 
		       		</td>
		         	<td colspan = 2 align="left" > 
						<% if( (nBlockidx+1)*nBlockcnt < nTotalpage ) { %>
							<table width="50">
		       					<tr>
		       						<td align="left"> <a href="mohList.jsp?page=<%=(nBlockidx+1)*nBlockcnt%>"> &#62; </a> </td>
		       						<td align="right"> <a href="mohList.jsp?page=<%=nTotalpage%>"> &#62;&#62; </a> </td>
		       					</tr>
		       				</table> 
						<% } %> 
		         	</td>
